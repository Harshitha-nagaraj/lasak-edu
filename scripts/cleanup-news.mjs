/**
 * Cleanup news collection - remove blog entries from news WITH AUTH
 * Run: node scripts/cleanup-news.mjs
 */
...
async function cleanup() {
    try {
        ...
        console.log("📡 Fetching documents from 'news' collection...");
        const ids = await listDocs('news');
        console.log(`Found ${ids.length} documents.`);

        for (const id of ids) {
            // Delete everything that isn't one of our new news-1, news-2, news-3 IDs
            // to be safe and ensure only the real news cards remain.
