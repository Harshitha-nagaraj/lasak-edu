/**
 * Cleanup video_testimonials collection - remove ALL entries
 * Run: node scripts/cleanup-video-testimonials.mjs
 */
...
async function cleanup() {
    try {
        ...
        console.log("📡 Fetching documents from 'video_testimonials' collection...");
        const ids = await listDocs('video_testimonials');
        console.log(`Found ${ids.length} documents.`);

        for (const id of ids) {
            await deleteDoc('video_testimonials', id);
        }

        console.log("✨ Video Testimonials cleanup complete!");
    } catch (err) {
