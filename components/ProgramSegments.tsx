...
const ProgramSegments = ({ segments }: { segments?: any[] }) => {
  ...
  return (
    <div className="w-full bg-slate-50 relative pb-[40vh]"> {/* Extremely large padding for last card clearance */}
          ...
          {displaySegments.map((program, i) => {
            return (
              ...
              >
                ...
                >
                  <div className="relative flex flex-col h-full p-5 md:p-10 overflow-y-auto custom-scrollbar">
                    <div className="w-full max-w-5xl mx-auto h-full flex flex-col justify-between min-h-full">
                      
                      {/* 1. Header Area */}
                      <div className="flex items-center gap-4 md:gap-8 flex-shrink-0">
                        <div className="w-12 h-12 md:w-16 md:h-16 flex-shrink-0 bg-white/20 backdrop-blur-lg border border-white/30 rounded-xl md:rounded-2xl flex items-center justify-center text-white p-2 md:p-4 shadow-xl">
                      ...
                      {/* 2. Description */}
                      <p className="text-white/90 text-[10px] md:text-base lg:text-lg my-3 md:my-5 max-w-4xl leading-relaxed font-bold opacity-95 line-clamp-2 md:line-clamp-3">
                        {program.description}
                      </p>

                      {/* 3. Autoscrolling Courses Ticker */}
                      <div className="w-full overflow-hidden relative py-2 my-2 flex-shrink-0">
                        <div className="flex animate-scroll hover:pause space-x-4 md:space-x-6">
                          {[...(program.categories || []), ...(program.categories || []), ...(program.categories || [])].map((cat: any, idx: number) => (
                            <div
                              key={idx}
                              className="flex-shrink-0 w-28 md:w-48 bg-white/10 backdrop-blur-3xl border border-white/20 p-3 md:p-5 rounded-[1.5rem] group cursor-pointer transition-all duration-500 hover:bg-white/20 shadow-lg"
                            >
                              <div className="w-6 h-6 md:w-10 md:h-10 rounded-lg bg-white/20 flex items-center justify-center text-white mb-2 md:mb-3 group-hover:bg-white group-hover:text-slate-900 transition-all duration-500">
                                <div className="w-4 h-4 md:w-6 md:h-6">
                                  {typeof cat.icon === 'string' ? getIcon(cat.icon) : <Star className="w-full h-full" />}
                                </div>
                              </div>
                              <h3 className="text-[8px] md:text-xs lg:text-sm font-black text-white leading-tight uppercase tracking-wider">{cat.name}</h3>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      {/* 4. CTA Buttons */}
                      <div className="flex flex-wrap gap-3 md:gap-6 pt-4 flex-shrink-0 pb-4">
                        <Link
                          to="/contact"
                          className="px-6 py-2.5 md:px-12 md:py-4 bg-white text-slate-900 font-black text-[10px] md:text-base rounded-xl hover:shadow-2xl transition-all duration-300 active:scale-95 flex items-center gap-2 group shadow-xl"
                        >
                        ...
                        <Link
                          to="/courses"
                          className="px-6 py-2.5 md:px-12 md:py-4 bg-transparent border-2 border-white text-white font-black text-[10px] md:text-base rounded-xl hover:bg-white/10 transition-all duration-300 active:scale-95 flex items-center gap-2 group"
                        >
                          Explore First
                        </Link>
                      </div>
