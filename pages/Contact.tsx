...
const Contact = () => {
  ...
  const fetchFormSettings = async () => {
    try {
      ...
      const db = await getFirestoreDb();
      const docSnap = await getDoc(doc(db, 'site_settings', 'contact_form_settings'));
      if (docSnap.exists()) {
        const data = docSnap.data();
        setFormSettings(data.value || data);
      }
    } catch (error) {
  ...
  const fetchPageSettings = async () => {
    try {
      ...
      const { doc, getDoc } = await import('firebase/firestore');
      const db = await getFirestoreDb();
      const docSnap = await getDoc(doc(db, 'site_settings', 'contact_page_content'));
      ...
      <div className="container mx-auto px-4 -mt-20 relative z-20">
        <div className="flex flex-col gap-12">
          ...
          <div className="flex justify-center">
            <div className={`w-full ${branches.length === 1 ? 'max-w-2xl' : 'grid grid-cols-1 md:grid-cols-2 gap-8'}`}>
            {branches.map((branch) => (
              <div key={branch.id} className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 border border-slate-200 shadow-2xl hover:shadow-blue-600/5 transition-all duration-500 group">
                {/* Card Header */}
                ...
                <div className="mt-10">
                  <a
                    href={branch.directions_url || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(branch.name + ' ' + branch.address)}`}
                    target="_blank"
